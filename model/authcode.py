...
from config import AUTHCODE_FONT as authcode_font

_letter_cases = "abcdefghjkmnpqrstuvwxy"  # 小写字母，去除可能干扰的i，l，o，z
_upper_cases = _letter_cases.upper()  # 大写字母
_numbers = ''.join(map(str, range(3, 10)))  # 数字
init_chars = ''.join((_letter_cases, _upper_cases, _numbers))
#authcode_font = os.path.join(path.dirname(path.abspath(__file__)), '../static/fonts/arialbi.ttf')
fontType = authcode_font
...
if __name__ == "__main__":
    ...
    image_path = path.join(path.dirname(path.abspath(__file__)), '..\\static\\verify_img\\')
    code_img[0].save(image_path + code_img[1] + ".gif", "GIF")
    print(code_img[1])
